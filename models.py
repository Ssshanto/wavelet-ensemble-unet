...
import numpy as np
from helper import *
from layers import unetConv2, unetUp_origin, waveletUp_origin
...
class UNet_Bilinear(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_Bilinear, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
...
class UNet3Plus(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, bilinear=True, feature_scale=4,
        ...
        # output
        self.outconv1 = OutConv(self.UpChannels, n_classes)
...
class R2U_Net(nn.Module):
    def __init__(self,n_channels=3,n_classes=1,t=2):
        ...
        self.Conv_1x1 = OutConv(64, n_classes)
...
class R2AttU_Net(nn.Module):
    def __init__(self,n_channels=3,n_classes=1,t=2):
        ...
        self.Conv_1x1 = OutConv(64,n_classes,kernel_size=1,stride=1,padding=0)
...
class WS_UNet2Plus(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, bilinear=True, feature_scale=4, 
                 is_deconv=True, is_batchnorm=True, is_ds=True):
        super(WS_UNet2Plus, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        self.feature_scale = feature_scale
        ...
        # downsampling
        self.conv00 = unetConv2(self.n_channels, filters[0], self.is_batchnorm)
        self.maxpool0 = WaveletDownsampling()
        self.conv10 = unetConv2(filters[0], filters[1], self.is_batchnorm)
        self.maxpool1 = WaveletDownsampling()
        self.conv20 = unetConv2(filters[1], filters[2], self.is_batchnorm)
        self.maxpool2 = WaveletDownsampling()
        self.conv30 = unetConv2(filters[2], filters[3], self.is_batchnorm)
        self.maxpool3 = WaveletDownsampling()
        self.conv40 = unetConv2(filters[3], filters[4], self.is_batchnorm)

        # upsampling
        self.up_concat01 = waveletUp_origin(filters[1], filters[0], self.is_deconv)
        self.up_concat11 = waveletUp_origin(filters[2], filters[1], self.is_deconv)
        self.up_concat21 = waveletUp_origin(filters[3], filters[2], self.is_deconv)
        self.up_concat31 = waveletUp_origin(filters[4], filters[3], self.is_deconv)

        self.up_concat02 = waveletUp_origin(filters[1], filters[0], self.is_deconv, 3)
        self.up_concat12 = waveletUp_origin(filters[2], filters[1], self.is_deconv, 3)
        self.up_concat22 = waveletUp_origin(filters[3], filters[2], self.is_deconv, 3)

        self.up_concat03 = waveletUp_origin(filters[1], filters[0], self.is_deconv, 4)
        self.up_concat13 = waveletUp_origin(filters[2], filters[1], self.is_deconv, 4)

        self.up_concat04 = waveletUp_origin(filters[1], filters[0], self.is_deconv, 5)
...
class WS_UNet3Plus(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, bilinear=True, feature_scale=4,
                 is_deconv=True, is_batchnorm=True):
        super(WS_UNet3Plus, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        self.feature_scale = feature_scale
        ...
        self.conv4 = unetConv2(filters[2], filters[3], self.is_batchnorm)
        self.maxpool4 = WaveletDownsampling()
...
class WS_AttU_Net(nn.Module):
    def __init__(self,n_channels=3,n_classes=1):
        super(WS_AttU_Net,self).__init__()
        
        self.Maxpool = WaveletDownsampling()
        ...
        self.Up5 = wavelet_up_conv(ch_in=1024,ch_out=512)
        self.Att5 = Attention_block(F_g=512,F_l=512,F_int=256)
        self.Up_conv5 = conv_block(ch_in=1024, ch_out=512)

        self.Up4 = wavelet_up_conv(ch_in=512,ch_out=256)
        self.Att4 = Attention_block(F_g=256,F_l=256,F_int=128)
        self.Up_conv4 = conv_block(ch_in=512, ch_out=256)
        
        self.Up3 = wavelet_up_conv(ch_in=256,ch_out=128)
        self.Att3 = Attention_block(F_g=128,F_l=128,F_int=64)
        self.Up_conv3 = conv_block(ch_in=256, ch_out=128)
        
        self.Up2 = wavelet_up_conv(ch_in=128,ch_out=64)
        self.Att2 = Attention_block(F_g=64,F_l=64,F_int=32)
        self.Up_conv2 = conv_block(ch_in=128, ch_out=64)
...
class WI_UNet2Plus(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, bilinear=True, feature_scale=4, 
                 is_deconv=True, is_batchnorm=True, is_ds=True):
        super(WI_UNet2Plus, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        self.feature_scale = feature_scale
        ...
        self.wavelet_downsample = WaveletDownsampling(wavelet='db1', level=1)

        # downsampling
        self.conv00 = unetConv2(self.n_channels, filters[0], self.is_batchnorm)
        self.maxpool0 = nn.MaxPool2d(kernel_size=2)
        self.concat1 = MRConcat(self.n_channels, filters[0])
        self.conv10 = unetConv2(filters[0], filters[1], self.is_batchnorm)
        self.maxpool1 = nn.MaxPool2d(kernel_size=2)
        self.concat2 = MRConcat(self.n_channels, filters[1])
        self.conv20 = unetConv2(filters[1], filters[2], self.is_batchnorm)
        self.maxpool2 = nn.MaxPool2d(kernel_size=2)
        self.concat3 = MRConcat(self.n_channels, filters[2])
        self.conv30 = unetConv2(filters[2], filters[3], self.is_batchnorm)
        self.maxpool3 = nn.MaxPool2d(kernel_size=2)
        self.concat4 = MRConcat(self.n_channels, filters[3])
        self.conv40 = unetConv2(filters[3], filters[4], self.is_batchnorm)
    ...
    def forward(self, inputs):
        ...
        downsampled2 = self.wavelet_downsample(inputs)
        downsampled3 = self.wavelet_downsample(downsampled2)
        downsampled4 = self.wavelet_downsample(downsampled3)
        downsampled5 = self.wavelet_downsample(downsampled4)
...
class WI_UNet3Plus(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, bilinear=True, feature_scale=4,
                 is_deconv=True, is_batchnorm=True):
        super(WI_UNet3Plus, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        self.feature_scale = feature_scale
    ...
    def forward(self, inputs):
        ...
        h2 = self.maxpool1(h1)
        h2 = self.concat1(downsampled2, h2)
        h2 = self.conv2(h2)  # h2->160*160*128
        

        h3 = self.maxpool2(h2)
        h3 = self.concat2(downsampled3, h3)
        h3 = self.conv3(h3)  # h3->80*80*256

        h4 = self.maxpool3(h3)
        h4 = self.concat3(downsampled4, h4)
        h4 = self.conv4(h4)  # h4->40*40*512

        h5 = self.maxpool4(h4)
        h5 = self.concat4(downsampled5, h5)
        hd5 = self.conv5(h5)  # h5->20*20*1024
...
class WI_AttU_Net(nn.Module):
    def __init__(self,n_channels=3,n_classes=1):
        super(WI_AttU_Net,self).__init__()
        
        self.Maxpool = nn.MaxPool2d(kernel_size=2,stride=2)
        self.wavelet_downsample = WaveletDownsampling(wavelet='db1', level=1)

        self.concat1 = MRConcat(n_channels, 64)
        self.concat2 = MRConcat(n_channels, 128)
        self.concat3 = MRConcat(n_channels, 256)
        self.concat4 = MRConcat(n_channels, 512)
